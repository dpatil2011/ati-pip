...
class Pin extends Component {
  ...
  changeText(newText) {
    ...
    if(newText.length==6){
      ...
      if(!error){
        this.props.navigate('CashOutSuccess')
      }
    }
	}
  render() {
    ...
    return (
      <View style={{flex:1,backgroundColor:'#a92930'}}>
        ...
        <Left><TouchableOpacity><AtiText style={{ marginLeft:wp('6.4%'),marginTop:wp('7.1%'),}}> <Icon name="arrow-back" size={24} color="white" /></AtiText></TouchableOpacity></Left>
...
export default connect(mapStateToProps, mapActionToProps())(withWebConsole(Pin));