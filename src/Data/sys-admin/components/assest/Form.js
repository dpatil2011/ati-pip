...
class Form extends React.Component {
    ...
    constructor(props){
      ...
      componentDidMount() {
        ...
        fetch("http://192.168.0.51:5555/category?type=it&status=1")
        ...
        //   fetch("http://192.168.0.151:5555/asset_form/1")
        //   // when we get a response map the body to json
        //   .then(response => response.json())
        //   // and update the state data to said json
    ...
    handleSubmit = () =>{
        ...
        this.state.data.map(function(login){
         ...
         if(check){
          axios({
            method: 'post',
            url: 'http://192.168.0.51:5555/login/',
  ...
  filter=()=>{
    var id = $('#request_type').val();
     ...
     var catid=id[0];
     var size=id[1];
     var iid= sessionStorage.getItem('SysId')
        if(iid==""){
            window.location="/"
        }
    fetch("http://192.168.0.151:5555/asset_form?catId="+catid)
    // when we get a response map the body to json
    .then(response => response.json())
    // and update the state data to said json
    .then(formData => this.setState({ formData }));
    ...
    insert=()=>{
     ...
     if(size<=5){
      ...
      window.setTimeout(()=>{
        var url = "http://192.168.0.151:5555/available_assets";
        fetch(url, {
