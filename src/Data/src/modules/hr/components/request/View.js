...
class ViewPending extends Component{
    ...
    componentDidMount() {        
        ...
        fetch("http://localhost:5555/user/"+userId)
        ...
        // to specified URL
        fetch("http://localhost:5555/request/"+reqId)
          // when we get a response map the body to json
          .then(response => response.json())
          // and update the state data to said json
 ...
 // console.log(data);
   ...
   console.log(this.state.request[id-1])
      ...
      fetch("http://localhost:5555/request/"+id, {
  ...
  .then(response => response.json().then(request=>{
      fetch("http://localhost:5555/request/"+id).then(response => response.json().then(request=>{
          this.setState({
            request: request,
      ...
      style={{backgroundColor:'red',borderColor:'red'}}
        ...
        ];
        {
            const dataList = [
                    ...
                    {
                        title:"Closed On",
                        description:this.state.request.cd
                    },
                    {
                        title:"Email Address",
