       ...
       if(validate==i)
        {
          axios({
            ...
            data: {catName,fields,assets,status}, // you are sending body instead
            headers: {
...
this.setState({
  ...
  actvateCategory=()=>{
   ...
   var catId = sessionStorage.getItem('activateCatId');
    ...
    var catSet = this.state.category[catId-1];
    var status = '1';
    var catName = catSet.catName;
    var fields =  catSet.fields;
    var assets= catSet.assets;
    axios({
      method: 'PUT',
      url: 'http://localhost:5555/category/'+catId,
      data: {assets,fields,catName,status}, // you are sending body instead
      headers: {
        // 'Authorization': `bearer ${token}`,
      'Content-Type': 'application/json'
      }, 
    ...
    render(){
     ...
     return(
      ...
      <div className="tile" id = "activate" style={{display:'none'}}>
            <a href="#" onClick={showActivateForm}> {'<<'} <b>Go Backrr</b> </a>
